...
- Article ranking based on freshness, source signal, metadata richness, and launch/breakthrough cues.
- Safer compatibility for monitors expecting `article.content`.
- Better breaking-news prioritization from source trust and summary richness.
...
2. Event-loop friendly threaded parsing.
3. Concurrency limits to avoid burst overload.
4. URL normalization for deduplication.
5. Freshness-aware ranking.
6. Domain-weight ranking.
7. Summary-richness ranking.
8. Breaking-news keyword reinforcement.
9. Compatibility fallback from `content` to `summary`.
10. Title-signature deduplication in the real-time monitor.
