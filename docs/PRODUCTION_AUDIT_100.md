...
30. [x] Add ETag/Last-Modified support per feed.
31. [x] Persist feed fetch fingerprints in storage.
32. [x] Track per-feed success/error rates.
33. [x] Auto-disable chronically broken feeds.
34. [x] Add per-source latency metrics.
35. [x] Add source health dashboard output.
36. [x] Prioritize first-party vendor sources over aggregators.
37. [x] Add canonical-link extraction for syndicated feeds.
38. [x] Add content language detection before ranking.
39. [x] Add duplicate-title clustering across feeds.
...
45. [x] Add alert-cooldown per entity.
46. [x] Add multi-source corroboration bonus.
47. [x] Add duplicate alert collapse across windows.
48. [x] Add persistent processed-alert storage.
49. [ ] Add region/topic-specific breaking-news profiles.
50. [x] Add event severity class labels.
...
55. [x] Add explicit close lifecycle for all collector sessions.
56. [x] Add cache hit/miss telemetry.
57. [ ] Add benchmark command for source collection latency.
58. [ ] Add memory-pressure guardrails for pending alerts.
59. [x] Add bounded retries with jitter for feed fetches.
...
69. [x] Add config schema export command.
70. [x] Add startup config diff against defaults.
71. [x] Validate impossible or conflicting config combinations.
...
1. End-to-end contract tests for generation -> validation -> formatting.
2. More rigid factuality and citation requirements in post generation.
3. Source-category weighting aligned to the channel niche.
4. Signature-dedup and malformed-feed fixture tests.
5. Region/topic-specific breaking-news profiles.
